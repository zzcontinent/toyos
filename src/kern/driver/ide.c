...
static struct ide_device {
	u8 valid;        // 0 or 1 (If Device Really Exists)
	u32 sets;          // Commend Sets Supported
...
void ide_init(void)
{
	...
	for (ideno = 0; ideno < MAX_IDE; ideno ++)
	{
		...
		u32 buffer[128];
		insl(iobase + ISA_DATA, buffer, sizeof(buffer) / sizeof(u32));
		...
		if (cmdsets & (1 << 26)) {
			sectors = *(u32 *)(ident + IDE_IDENT_MAX_LBA_EXT);
		}
		else {
			sectors = *(u32 *)(ident + IDE_IDENT_MAX_LBA);
		}
		ide_devices[ideno].sets = cmdsets;
		ide_devices[ideno].size = sectors;

		/* check if supports LBA */
		assert((*(u16 *)(ident + IDE_IDENT_CAPABILITIES) & 0x200) != 0);
