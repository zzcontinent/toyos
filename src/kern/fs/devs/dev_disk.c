...
static char *disk0_buffer; static semaphore_t disk0_sem;
...
static void disk0_read_blks_nolock(uint32_t blkno, uint32_t nblks)
{
	...
	if ((ret = ide_read_secs(DISK0_DEV_NO, sectno, disk0_buffer, nsecs, SECTSIZE)) != 0) {
...
static void disk0_write_blks_nolock(uint32_t blkno, uint32_t nblks)
{
	int ret;
	uint32_t sectno = blkno * DISK0_BLK_NSECT, nsecs = nblks * DISK0_BLK_NSECT;
	if ((ret = ide_write_secs(DISK0_DEV_NO, sectno, disk0_buffer, nsecs, SECTSIZE)) != 0) {
